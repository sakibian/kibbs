  ...
  if (messenger != 0) {
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        $(".m-header-messaging .user-name").html(data.fetch.is_admin);
        $("#chatUser").html(data.fetch.name)
        if(data.fetch.is_admin == 0) {
          $("#chatUserType").html('(Member)')
        } else {
          $("#chatUserType").html('(Admin)')
        }
